...
Array *GetNeighbours(Grafo g, u32 i) {
    Array *ns = ArrayNew();
    for (u32 x = 0; x < Grado(i, g); x++) {
        u32 neigh = OrdenVecino(x, i, g);
        if (neigh == U32_MAX) {
            ArrayDestroy(ns);
            return NULL;
        }
...
bool TestOrdenVecino(void) {
    ...
    u32 nodes[] = {0, 1, 2, 3, 4, 5, 6, 7};
    struct {
        u32 x;
        u32 y;
    } cons[] = {{1, 4}, {2, 5}, {2, 6}, {3, 5}, {3, 6}, {7, 6}};
    u32 v_length = 8;
    u32 e_length = 6;
    ...
    Array *neighbours = GetNeighbours(g, 1);
    if (neighbours == NULL) {
        log_error("%s: Error getting neighbours", __func__);
        GraphDestroy(g);
        return false;
    }
    if (neighbours->length != 1) {
        log_error("%s: Bad neighbour count!", __func__);
        arrayPrint(neighbours);
        ArrayDestroy(neighbours);
        GraphDestroy(g);
        return false;
    }
    u32 neigh_order = ArrayGet(neighbours, 0);
    if (Nombre(neigh_order, g) != 4) {
        log_error("%s: Bad neighbour!", __func__);
        arrayPrint(neighbours);
        ArrayDestroy(neighbours);
        GraphDestroy(g);
        return false;
    ...
    neigh_order = ArrayGet(neighbours, 0);
    if (Nombre(neigh_order, g) != 4) {
        log_error("%s: Bad neighbour! Found %u instead of 4", __func__,
                  Nombre(neigh_order, g));
        arrayPrint(neighbours);
        ArrayDestroy(neighbours);
        GraphDestroy(g);
        return false;
...
int main(void) {
    ...
    bool (*tests[])(void) = {TestNaturalOrder, TestNaturalTranslate,
                             ...
                             TestRevierteBC,   TestOrdenVecino};
